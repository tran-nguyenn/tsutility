...
def random_forest(df):
    ...
    :return:
    """
    dataset = spark.createDataFrame(ts_cov)
    
    # bucketize the dependent variable
    splits = [0.0,0.3,0.6,1, float("inf")]
    bucketizer = Bucketizer(splits=splits, inputCol = Quantatative_Response ,outputCol='bm_wfa_buckets')
    bucketedData = bucketizer.transform(dataset)
    
    
    mlreadyData = bucketedData.select(*datcol)
...
def filter_ml(df):
    ...
    """
    # filter dataset
    keepCols = ['material','location','sales_org','distribution_channel', 'winning_model_wfa','bm_wfa_buckets', 'coeff_of_variation', 'covXYZ','dsXYZ', 'rawcovXYZ', 'descovXYZ', 'raw_cov', 'deseasonalized_cov']
